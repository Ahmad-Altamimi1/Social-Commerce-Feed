...
router.get("/cart", async (req, res) => {
  ...
  res.json(await buildCart(actorId));
});
...
router.post("/cart", async (req, res) => {
  ...
  const existing = await db.select().from(cartItemsTable).where(
    and(eq(cartItemsTable.userId, actorId), eq(cartItemsTable.productId, productId))
  );
  if (existing[0]) {
    await db.update(cartItemsTable).set({ quantity: existing[0].quantity + quantity }).where(eq(cartItemsTable.id, existing[0].id));
  } else {
    await db.insert(cartItemsTable).values({ userId: actorId, productId, merchantId: product.merchantId, quantity });
  }
  res.json(await buildCart(actorId));
});
...
router.put("/cart/:itemId", async (req, res) => {
  ...
  if (quantity <= 0) {
    ...
    await db.update(cartItemsTable).set({ quantity }).where(and(eq(cartItemsTable.id, itemId), eq(cartItemsTable.userId, actorId)));
  }
  res.json(await buildCart(actorId));
});
