...
export default function DashboardPage() {
  ...
  const { data: merchant, isLoading: merchantLoading } = useGetMyMerchantProfile({
    query: { queryKey: ["/api/merchant/me"], enabled: isAuthenticated },
  });
  const { data: products, refetch: refetchProducts } = useListMyProducts({
    query: { queryKey: ["/api/merchant/products"], enabled: !!merchant },
  });
  const { data: orders } = useGetMerchantOrders({
    query: { queryKey: ["/api/merchant/orders"], enabled: !!merchant },
  });
          ...
          <div className="px-4 py-4 space-y-4">
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { label: "Rating", value: (merchant.rating ?? 0) > 0 ? `${merchant.rating}★` : "—", sub: `${merchant.reviewCount ?? 0} reviews` },
