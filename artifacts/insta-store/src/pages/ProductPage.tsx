...
export default function ProductPage() {
  ...
  const handleShare = async () => {
    if (!product) return;
    const productUrl = `${window.location.origin}${import.meta.env.BASE_URL}product/${product.slug}`;
    let shared = false;
    if (navigator.share) {
      try {
    ...
    return (
      <div className="flex flex-col min-h-screen max-w-[428px] mx-auto bg-background">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4 p-8 text-center">
          ...
          <button onClick={() => navigate("/")} className="px-4 py-2 bg-primary text-primary-foreground rounded-full font-medium text-sm">
            Back to feed
          </button>
        </div>
      </div>
